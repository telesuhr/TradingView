#!/usr/bin/env python3
"""
上海銅先物SCFc系RIC包括テスト - データ可用性と期間構造分析
"""
...
def test_shanghai_copper_futures():
    """上海銅先物SCFc系RIC包括テスト"""
    ...
    # 上海銅先物RIC候補（第1-12限月）
    shanghai_copper_futures = {}
    for i in range(1, 13):  # SCFc1からSCFc12まで
        shanghai_copper_futures[f"SCFc{i}"] = {
            "ric": f"SCFc{i}",
            "name": f"上海銅先物第{i}限月",
            "month_num": i
        }
    ...
    print("=" * 80)
    print("上海銅先物SCFc系RIC包括テスト - データ可用性と期間構造分析")
    print("=" * 80)
    ...
    for contract_code, info in shanghai_copper_futures.items():
        ric = info["ric"]
        name = info["name"]
        ...
        try:
            ...
            if data is not None and not data.empty:
                ...
                if pd.notna(last_price) and last_price is not None:
                    print(f"  ✓ 最新価格: {last_price:,.0f} CNY/MT")
                    print(f"  ✓ 日付: {last_date}")
                    
                    # 価格詳細
                    price_info = []
                    if pd.notna(open_price):
                        price_info.append(f"始値: {open_price:,.0f}")
                    if pd.notna(high_price):
                        price_info.append(f"高値: {high_price:,.0f}")
                    if pd.notna(low_price):
                        price_info.append(f"安値: {low_price:,.0f}")
                    if pd.notna(close_price):
                        price_info.append(f"終値: {close_price:,.0f}")
                    
                    if price_info:
                        print(f"  ✓ 価格詳細: {', '.join(price_info)}")
                        ...
                        if ts_data is not None and not ts_data.empty:
                            ...
                            if len(close_series) > 0:
                                ...
                                if len(volume_series) > 0:
                                    ...
                                    print(f"  ✓ 7日平均価格: {avg_price_7d:,.0f} CNY/MT")
                                    print(f"  ✓ 7日平均出来高: {avg_volume_7d:,.0f} 契約")
                                    print(f"  ✓ 7日標準偏差: {std_price_7d:.0f} CNY/MT")
                                else:
                                    avg_volume_7d = 0
                                    print(f"  ✓ 7日平均価格: {avg_price_7d:,.0f} CNY/MT")
                                ...
                                # 期間構造用の価格保存
                                prices_for_curve[month_num] = last_price
    ...
    if successful_contracts:
        print(f"\n✓ 有効契約: {len(successful_contracts)}/{len(shanghai_copper_futures)}")
        ...
        if len(prices_for_curve) >= 3:
            ...
            for month, price in sorted_months:
                contract_code = f"SCFc{month}"
                volume = results.get(contract_code, {}).get('volume', 0)
                print(f"    第{month}限月: {price:,.0f} CNY/MT (出来高: {volume:,.0f})")
            ...
            if len(sorted_months) >= 2:
                ...
                if price_diff > 100:  # 100 CNY/MT以上の差
                    structure = "コンタンゴ"
                ...
                print(f"\n  期間構造判定:")
                print(f"    第{near_month}限月 vs 第{far_month}限月: {price_diff:+,.0f} CNY/MT")
                print(f"    月間スロープ: {monthly_slope:+.0f} CNY/MT/月")
        ...
        for i, (contract, data, score) in enumerate(ranking_data[:6], 1):
            ...
            print(f"     価格: {data['last_price']:,.0f} CNY/MT")
...
if __name__ == "__main__":
    try:
        results, successful, failed, curve_data = test_shanghai_copper_futures()
