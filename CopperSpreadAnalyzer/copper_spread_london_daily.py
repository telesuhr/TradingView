#!/usr/bin/env python3
"""
LME Copper スプレッド取引データ取得（ロンドン時間基準・日別）
全スプレッド組み合わせの取引データをロンドン時間の1日単位で取得
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('logs/copper_spread_london_daily.log'),
...
def get_london_business_day(offset_days=1):
    ...
    # ロンドン営業日の開始・終了時刻（UTC換算）
    london_date = target_date.date()
    
    # ロンドン時間の1日の開始（00:00）と終了（23:59:59）
    start_london = london_tz.localize(datetime.combine(london_date, datetime.min.time()))
    end_london = london_tz.localize(datetime.combine(london_date, datetime.max.time()))
    
    # UTCに変換
    start_utc = start_london.astimezone(pytz.UTC)
    end_utc = end_london.astimezone(pytz.UTC)
    
    return {
        'london_date': london_date,
        'start_utc': start_utc,
        'end_utc': end_utc,
        'start_london': start_london,
        'end_london': end_london
    }
...
def get_spread_data_london_daily(spread_combinations, london_date_info, logger, max_spreads=None):
    ...
    start_utc = london_date_info['start_utc']
    end_utc = london_date_info['end_utc']
    ...
    logger.info(f"UTC期間: {start_utc.strftime('%Y-%m-%d %H:%M')} ～ {end_utc.strftime('%Y-%m-%d %H:%M')}")
...
def main():
    ...
    logger.info("LME Copper スプレッド取引データ取得（ロンドン時間基準・日別）開始")
